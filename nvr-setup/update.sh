...
http {
  server {
    ...
    location / {
      ...
      # Strip any CORS headers go2rtc emits so nginx is the sole source,
      # preventing duplicate header values that browsers reject.
      proxy_hide_header Access-Control-Allow-Origin;
      proxy_hide_header Access-Control-Allow-Methods;
      proxy_hide_header Access-Control-Allow-Headers;
      proxy_hide_header Access-Control-Expose-Headers;

      # Full CORS — required for HLS requests from dash.allonys.com
